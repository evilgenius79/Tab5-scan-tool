...
void screen_home_create(lv_obj_t* parent) {
    ...
    // Battery + MIL now live in the global top bar (shown on every screen).
...
void screen_home_update(void) {
    // Battery + MIL are rendered by the global top bar (ui_topbar_update),
    // so the home screen has no per-frame work of its own.
}
